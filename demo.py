    ...
    def plot_bboxes_1(self,image, bboxes,side='left'):
        ...
        for (l, t, r, b, cls_name, track_id) in bboxes:
            ...
            if track_id in this_ids_info and this_ids_info[track_id]['speed'] != 0:
                ...
                # if speed<50:
                #     # # 超速警告（红色背景 + 文字）
                #     warning_text = f"WARNING: {speed} km/h (Too Low!)"
                #     (text_width, text_height), _ = cv2.getTextSize(warning_text, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
                    
                #     # 画警告框（红色背景）
                #     cv2.rectangle(image, (10, 10), (20 + text_width, 40 + text_height), (0, 0, 255), -1)  # -1 表示填充
                #     cv2.putText(image, warning_text, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                if speed < 90:
                    ...
                    # # 超速警告（红色背景 + 文字）
                    warning_text = f"WARNING: {speed} km/h (Too Fast!)"
                    (text_width, text_height), _ = cv2.getTextSize(warning_text, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
